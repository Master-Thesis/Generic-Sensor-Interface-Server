...
#undef UNICODE
// ------------------------------------------------------------------------------------
static void _cdecl _TRACE (const char* fmt, ...)
	{
	...
	va_end (argptr);
        std::string s;
            ...
            for (unsigned int x=0; x<s.size(); x++)
                {
                    buf[x] = s.at(x);
                    std:: cout << s.at(x); // checks the char at index x (fine)
                }
...
const char*   wiimote::ButtonNameFromBit		 [TOTAL_BUTTON_BITS] =
                                                                { _T("Left") , _T("Right"), _T("Down"), _T("Up"),
                                                                  _T("Plus") , _T("??")   , _T("??")  , _T("??") ,
                                                                  _T("Two")  , _T("One")  , _T("B")   , _T("A") ,
                                                                  _T("Minus"), _T("??")   , _T("??")  , _T("Home") };

const char*   wiimote::ClassicButtonNameFromBit [TOTAL_BUTTON_BITS] =
...
bool wiimote::Connect (unsigned wiimote_index, bool force_hidwrites)
	...
	if(!dev_info) {
                qDebug() << "couldn't get device info";
		return false;
		}
	...
	while(SetupDiEnumDeviceInterfaces(dev_info, NULL, &guid, index, &didata))
		...
		if(!SetupDiGetDeviceInterfaceDetail(dev_info, &didata, didetail,
                       ...
                       qDebug() << "couldn't get devinterface info for " << index;
			break;
			}
		...
		if(HidD_GetAttributes(Handle, &attrib))
			...
			if(wiimote_index == FIRST_AVAILABLE)
                                qDebug() << ".. opening Wiimote %u:", wiimotes_found;
			else
                                qDebug() << ".. opening:";
			...
			if(Handle == INVALID_HANDLE_VALUE) {
                                qDebug() << ".... failed with err " << GetLastError();
				goto skip;
				}
			...
			if(force_hidwrites && !_HidD_SetOutputReport) {
                                qDebug() << ".. can't force HID writes (not supported)";
				force_hidwrites = false;
				}

			if(force_hidwrites)
                                qDebug() << ".. (HID writes forced)";
			else{
			...
			// still failed?
			if(!bStatusReceived) {
                                qDebug() << "output failed - wiimote is not connected (or confused).";
				Disconnect();
				goto skip;
				}
...
void wiimote::CalibrateAtRest ()
	...
	if(IsBalanceBoard()) {
                qDebug() << ".. removing 'at rest' BBoard offsets.";
...
unsigned __stdcall wiimote::ReadParseThreadfunc (void* param)
	...
	while(1)
		...
		if(remote.bConnectionLost)
			{
connection_lost:
                        qDebug() << "read thread: connection to wiimote was lost";
			remote.Disconnect();
...
void wiimote::SetReportType (input_report type, bool continuous)
	...
	if(IsBalanceBoard() && (type != IN_BUTTONS_BALANCE_BOARD))
        ...
        const char* name = TYPE2NAME(IN_BUTTONS)				:
...
// ------------------------------------------------------------------------------------
bool wiimote::Load16bitMonoSampleWAV (const char* filepath, wiimote_sample &out)
	{
...
// ------------------------------------------------------------------------------------
bool wiimote::Load16BitMonoSampleRAW (const char*   filepath,
                                                                          bool		     _signed,
                                                                          speaker_freq   freq,
                                                                          wiimote_sample &out)
        {
