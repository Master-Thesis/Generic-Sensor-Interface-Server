...
class wiimote : public wiimote_state
	...
	public: // data
		...
		// get the button name from its bit index (some bits are unused)
                static const char*		   ButtonNameFromBit [TOTAL_BUTTON_BITS];
                static const char*		GetButtonNameFromBit (unsigned index)
			{
		...
		// same for the Classic Controller
                static const char*		   ClassicButtonNameFromBit [TOTAL_BUTTON_BITS];
                static const char*		GetClassicButtonNameFromBit (unsigned index)
			{
			_ASSERT(index < TOTAL_BUTTON_BITS);
			if(index >= TOTAL_BUTTON_BITS)
                                return _T("[invalid index]");
	...
	public: // methods
		...
		//   .wav sample
                static bool Load16bitMonoSampleWAV	 (const char*   filepath,
											  wiimote_sample &out);
		//   raw 16bit mono audio data (can be signed or unsigned)
                static bool Load16BitMonoSampleRAW	 (const char*   filepath,
											  bool		     _signed,
											  speaker_freq   freq,
											  wiimote_sample &out);
